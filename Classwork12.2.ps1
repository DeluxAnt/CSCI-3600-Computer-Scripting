...
if ($param -lt 0) {
    if ($param -eq -1) {"Argument for d option is out of range" }   #4
    ...
    if ($u -eq 1) {$env:USERNAME}
        elseif ($u -gt 1) {"More than one type of -u used"}    #1
    if ($p -eq 1) {$PWD}
        elseif ($p -gt 1) {"More than one type of -p used"}     #2
    if ($param -eq 1) {"The day is Sunday"}
        elseif ($d -gt 1) {"More than one type of -d used"}    #3
